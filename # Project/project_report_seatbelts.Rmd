...
```
While the seatbelt law still has the expected sign and magnitude, eABS and aribag are not showing any effect (or even a slightly positive one?). One possible explanation for the missing effect of eABS and airbags is that it takes time for new cars that actually have these safety features to be adopted by consumers.
...
```{r prophet}
data_prophet = data_pretreat %>%
  rename(y = DriversKilled,
         ds = index)

data_prophet_treat = data_treat %>%
  rename(y = DriversKilled,
         ds = index)

pred_prophet = prophet::prophet(data_prophet)
treat_prophet = prophet::prophet(data_prophet_treat)

data_future_forecast = prophet::make_future_dataframe(pred_prophet, periods = 669+31)
forecast_prophet = predict(pred_prophet, data_future_forecast)

#MAPE = treat_prophet$history$y - forecast_prophet
plot(pred_prophet, forecast_prophet) +
  geom_line(data = pred_prophet$history, aes(x = ds, y = y)) +
  geom_line(data = treat_prophet$history, aes(x = ds, y = y))
```
```{r prophet components}
prophet::prophet_plot_components(pred_prophet, forecast_prophet)
```
