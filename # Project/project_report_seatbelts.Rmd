...
subtitle: Does forcing people to wear seatbelts save lives?
output:
  html_notebook: default
  html_document: default
header-includes: \usepackage{amssymb} \usepackage{mathtools}
---
...
Last, but not least, we split our data set into a pre- and posttreatment phase:
...
- $\beta|\textbf{y}, \sigma^2 \sim \mathcal{N}(b_n, B_n)$ with  
  $$\begin{align*}
  B_n &= (\frac{1}{\sigma^2}X'X+B_0^{-1})^{-1}\\
...
gibbs_nig <- function(y, X, B0_diag = 10^2, b0_mu = 0, c0 = 1, C0 = 1) {
  ...
  for (i in 2:(draws+burnin+1)) {
   ...
   ## beta sampling
    Bn = solve(1/res[i-1, p+1] * (t(X) %*% X) + B0_inv)
    bn = Bn %*%  (1/res[i-1, p+1] * t(X) %*% y + B0_inv %*% b0)
   ## block draw betas:
...
model {  
  ...
  beta[1] ~ normal(b0_mu, sigma*B0_diag);        // intercept
  for(i in 2:K) 
    beta[i] ~ normal(b0_mu, sigma*B0_diag);      // coefficients regressors
...
```{r accuracy stats vanilla regression}
pred = data_plot$mean
actual = with(data_seatbelts, log(DriversKilled) - log(kms))
print(paste0("RMSE = ", sqrt(mean((pred - actual)^2))))
print(paste0("MAPE = ", mean( abs((pred - actual)/actual))))
```
According to research by [Tofallis](https://papers.ssrn.com/sol3/papers.cfm?abstract_id=2635088) MAPE is systematically biased towards predictions that are too low. He recommends the log-accuracy-ratio to avoid bias, so we include the mean-log-accuracy-ratio for comparison:
```{r log-accuracy ratio}
print(paste0("MLAR = ", mean(log(pred/actual))))
```

**including acf und pacf ?**
...
**attention: sampler assumes time-series is going from 1 to last vector entry!**
...
comparison = matrix(as.numeric(NA), nrow=draws+burnin+1, ncol=4)
## sampling steps
...
for (i in 2:(draws+burnin+1)) {
  ...
  ## beta sampling
  Bn = solve(B0_inv + 1/sigma2 * t(X_star) %*% X_star)
  bn = solve(B0_inv + 1/sigma2 * t(X_star) %*% X_star) %*% (B0_inv %*% b0 + 1/sigma2 * t(X_star) %*% y_star)

  Bn_test = solve(1/sigma2 * t(X_star) %*% X_star + B0_inv)
  bn_test = Bn_test %*%  (1/sigma2 * t(X_star) %*% y_star + B0_inv %*% b0)
  
  comparison[i-1, 1] = sum((Bn - Bn_test)^2)
  comparison[i-1, 2] = sum((bn - bn_test)^2)
  ## block draw betas:
  res[i,1:p] <- mvtnorm::rmvnorm(1, bn, Bn)
  ...
  z_star = solve(1/P0 + 1/sigma2 * t(u_L1) %*% u_L1)
  p_star = solve(1/P0 + 1/sigma2 * t(u_L1) %*% u_L1) %*% (1/P0 * p0 + 1/sigma2 * t(u_L1) %*% u[-1])
  
  z_star_test = solve(1/sigma2 * t(u_L1) %*% u_L1 + 1/P0)
  p_star_test = z_star %*% (1/sigma2 * t(u_L1) %*% u[-1] + 1/P0 * p0)
  
  comparison[i-1, 3] = sum((z_star - z_star_test)^2)
  comparison[i-1, 4] = sum((p_star - p_star_test)^2)
  
  ## Important: we need to make sure that rho in [-1, 1]
  run = TRUE
  while(run) {
    res[i, p+2] = rnorm(1, p_star, z_star)
...
We will assume that the posterior is given by, abusing notation:
$$p(\beta, \sigma^2, \rho|y) = \mathcal{N}(y | \beta, \sigma^2, \rho) \times \mathcal{N}(\beta|\sigma^2) \times \mathcal{IG}(\sigma^2) \times \mathcal{B}(\rho)$$
meaning we assume that $\rho$ is independent of both $\beta$ and $\sigma$.
...
parameters {
  vector[K] beta;                         //regression parameters - column vector by default
  real<lower=0> sigma;                    //standard deviation
  real<lower = 0, upper = 1> rho;        //correlation coefficient error term
}
model {  
  real error;
  real rho_scaled;
  sigma ~ inv_gamma(c0, C0);
  rho ~ beta(4, 2);                      // so that scaled beta is centered at roughly 0.5
  
  rho_scaled = 2*rho - 1;                // scaling beta to live on (-1, 1)
  error = 0;
  ...
  for(n in 1:N) {
    y[n] ~ normal(X[n]*beta + error, sigma);      //if a is a matrix, a[m] picks out row m of that matrix
    
    error = rho_scaled * (y[n] - (X[n]*beta));
  }
}
```
...
```
```{r summary extended model}
broom::tidyMCMC(model_extended_vanilla, conf.int = T)
```

It turns out that rho is actually quite high with a value of around 0.85. So there is indeed quite some serial correlation in the errors.
```{r}
...
Both chains seem to mix well. So convergence should be fine. As a final plausibility check, let's compare the results with the ARIMAX estimates:
```{r ARIMAX estimates}
...
The MA(1) term is markedly smaller than in the Bayesian setting and even changing the prior to put more weight on parameter values around 0.47 does not change that. This is definitely worth looking into more closely. Other than that, the findings are pretty similar. Except for the PetrolPrice coefficient and the MA(1) term, all coefficients are about the same.

In the next section, we are going to explore count models and start off with the Poission model:

### Poisson regression

Let's specify a simple Poisson regression model:
```{r rstan glm poisson, message=0}
model_glm = stan_glm(formula = DriversKilled ~ PetrolPrice + VanKilled + law + month + time_index, offset = log(kms),
                      data = data_seatbelts,
                      family = "poisson",
                      prior = cauchy(0,2.5),
...
The effect of the seatbelt law is still negative, although the far right tip of the distribution includes 0.

How robust is the effect if we add additional explanatory variables?
```{r cound model with airbag + abs}
model_glm_ext = stan_glm(formula = DriversKilled ~ PetrolPrice + VanKilled + law + 
                           eABS + airbag +  month + time_index, offset = log(kms),
                      data = data_seatbelts,
                      family = "poisson",
                      prior = cauchy(0,2.5),
                      prior_intercept = cauchy(0,10),
...
summary(model_glm_ext)
```
While the seatbelt law still has the expected sign and magnitude, eABS and aribag are not showing any effect (or even a slightly positive one?).
```{r cound regression in-sample fit}
y_rep = posterior_predict(model_glm_ext)
dim(y_rep)
```

We got `r dim(y_rep)[1]` simulated points for each observation, in our case 192. Let's do some plots:
```{r count diagnostic plots}
data_plot = as.data.frame(y_rep) %>%
  tidyr::gather(., key = index, value = val, 1:192) %>%
  mutate(index = as.numeric(index)) %>%
  group_by(index) %>%
...
ggplot(data_plot) +
  ...
  geom_line(aes(x = index, y = mean), col = "blue") +
  ## actual series
  geom_line(data = data_seatbelts, aes(x = time_index, y = DriversKilled)) +
  ggtitle("Posterior mean (blue) tracks actual series (black) closely",
          "Ribbon shows Q0.01 - Q0.99 credible interval")
```
```{r accuracy stats count regression}
pred = data_plot$mean
actual = with(data_seatbelts, DriversKilled)
print(paste0("RMSE = ", sqrt(mean((pred - actual)^2))))
print(paste0("MAPE = ", mean( abs((pred - actual)/actual))))
print(paste0("MLAR = ", mean(log(pred/actual))))
```

It appears that the prediction error is larger when we use the count model as compared to the vanilla Bayes model before.

Maybe a negative-binomial model performs better?
```{r negative binomial with airbag + abs}
model_glm_negb = stan_glm(formula = DriversKilled ~ PetrolPrice + VanKilled + law + 
                           eABS + airbag +  month + time_index, offset = log(kms),
                      data = data_seatbelts,
...
summary(model_glm_negb)
```
```{r negb regression in-sample fit}
y_rep = posterior_predict(model_glm_negb)
dim(y_rep)
```

```{r diagnostic plots negb}
data_plot = as.data.frame(y_rep) %>%
  tidyr::gather(., key = index, value = val, 1:192) %>%
  mutate(index = as.numeric(index)) %>%
  group_by(index) %>%
...
```{r accuracy stats negb regression}
pred = data_plot$mean
actual = with(data_seatbelts, DriversKilled)
print(paste0("RMSE = ", sqrt(mean((pred - actual)^2))))
print(paste0("MAPE = ", mean( abs((pred - actual)/actual))))
...
The fit does not really improve when we use the negative-binomial model, so overdispersion does not seem to be an issue, which makes intuitive sense just by looking at the plots.
...
- can I simply go back to #DriversKilled with Bayesian regression although we estimated log()? ->YES! We have sample estimates and we can transform them however we like
- posterior predictive distribution?

### Questions:
- Rhat -> what happens if there is only one chain? is the variance of parts of the chain checked against itself?
- Serial correlation -> correctly estimated?
- Why do we assume that the betas are all independent of each other?
- z_star, B0_star ... sigma^2 or sigma?
- How do we know that we can simply use the N(.,.) formula for the betas again for rho? 