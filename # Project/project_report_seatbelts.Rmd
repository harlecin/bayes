...
y_star = (y - rho*lag(y, 1))[-1] # remove NA at beginning
X_star = (X - rho*lag(X, 1))[-1]

## precalculating values
B0_inv = solve(B0)
### (X*'X*+B0_inv)^(-1)
pre1 = solve(crossprod(X_star) + B0_inv)

### (X*'y* + B0_inv*b0)
pre2 = t(X_star) %*% y_star + B0_inv %*% b0
...
for (i in 2:(draws+burnin+1)) {
   ...
   beta_i = res[i, 1:p]
   sigma2_i = res[i-1, p+1]
   y_hat_L1 = head(X %*% beta_i, -1)
   
   u = y[-1] - y_hat_L1 
   u_L1 = tail(u, -1)
   
   ### Note: u is of class 'matrix' and u[-1] is of class 'numeric' so we do not need t(u[-1]) for
   ### the vector multiplication: u[-1] %*% u_L1 == t(u[-1]) %*% u_L1
   p_star = solve(1/P0 + 1/sigma2_i * u[-1] %*% u_L1) %*% (1/P0 * p0 + 1/sigma2_i *  t(u[-1]) %*% u_L1)
   z_star = solve(1/P0 + 1/sigma2_i * t(u_L1) %*% u_L1)

   ## Important: we need to make sure that rho in [-1, 1]
   run = TRUE
   while(run) {
    res[i, 18] = rnorm(1, p_star, z_star)
    run = res[i, 18] < -1 | res[i, 18] > 1
   }
   
   # sigma^2 sampling
   Cn = C0 + .5*(crossprod(y_star - X_star %*% res[i, 1:p]) + t(res[i, 1:p] - b0) %*% B0_inv %*% (res[i, 1:p] - b0))
   res[i, p+1] <- 1/rgamma(1, shape = pre3, rate = Cn)
}
```
