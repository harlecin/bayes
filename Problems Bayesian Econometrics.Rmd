...
2. Calculate the acceptance ratio $R = min(1, r)$: $$r = \frac{f(\theta^*|y)}{f(\theta^{(s)}|y)} = \frac{f(y|\theta^*)f(\theta^*)}{f(y|\theta^{(s)})f(\theta^{(s)})}$$ where in our case $f(.|.)$ as defined above
...
for (i in 2:k) {
  x_proposal = runif(1, min = x[i-1] - delta, max = x[i-1] + delta)
  log_r = dexp(x_proposal, rate = y[i-1], log = 1) - dexp(x[i-1], rate = y[i-1], log = 1)
  accept = log(runif(1, 0, 1)) < log_r
  x[i] = if (accept) x_proposal else x[i-1]

  y_proposal = runif(1, min = y[i-1] - delta, max = y[i-1] + delta)
  log_r = dexp(y_proposal, rate = x[i-1], log = 1) - dexp(y[i-1], rate = x[i-1], log = 1)
  accept = log(runif(1, 0, 1)) < log_r
  y[i] = if (accept) y_proposal else y[i-1] 
}
